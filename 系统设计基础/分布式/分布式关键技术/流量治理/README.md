 ...
 * @LastEditTime: 2025-05-20 16:16:54
...
-->
# 流量治理
流量治理不同于服务治理，服务治理属于系统本身的事，而流量治理不仅仅属于系统本身需要处理的事情，系统之外接入层的流量也需要治理，另外，从层级来说，服务治理属于数据中心本身的治理行为，然而流量治理更多的应该是属于边缘计算，例如 CDN，它更多的应该属于数据中心之外的调控行为。


## 流量调控的主要功能
我们对于流量调控系统的核心需求是，首先，在无需人工调控的状态下，提升系统的稳定性，比如限流组件。其次在系统遭遇突发状态下力挽狂澜，仍旧保护系统的运行。说白了核心就是提升了系统的稳定性以及高可用性。

所以通常我们使用 API 网关来调控流量，或者使用 k8s + istio 去调控流量。
...
## 分布式事务一致性的问题
**分布式事务的核心问题是 ​ 分布式数据一致性问题** ​​。在跨节点、跨服务的复杂交互中，如何保证所有参与方的数据状态最终一致，是分布式事务设计的核心挑战


要解决数据不丢失的问题，只能通过数据冗余的方法，就算是数据分区，每个区也需要进行
数据冗余处理。这就是数据副本

在解决数据副本间的一致性问题时，我们有一些技术方案：

- master - master 双主互为备份方案
- master - slave 单主多从方案
- 两阶段提交
- paxos 算法/raft 算法

凡是通过业务补偿，或是在业务应用层上做的分布式事务的玩法，基本上都是两阶段提交，基本上，在应用层上解决事务问题，只有 “两阶段提交” 这样的方式

而在数据层解决事务问题，paxos、raft 都是解决分布式事务一致性的主要方案。

不过本质上两阶段提交其实就是 paxos 和 raft 这类强一致性算法的底层实现。

下面是**分布式事务的常见应用场景**：

- ​​ 强一致性协议 paxos，raft​​：(代价：牺牲可用性 CAP 定理中的 C 与 A 冲突
...
## 流量治理生产级软件
istio，服务网格，提供了全方面的流量治理功能，比如双向认证，流量控制，流量限流，流量分流，流量监控等。

如果不使用服务网格，那么其实流量控制的相关工作应该是网关的工作，比如使用 API 网关 kong 也可以完成这部分的工作。
