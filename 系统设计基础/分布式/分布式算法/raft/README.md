 ...
 * @LastEditTime: 2024-11-24 00:45:25
...
- raft 并不是所有节点都可以当选领导者，multi paxos 是所有节点都有选举的权利，在 raft 中**只有日志最完整的节点才能当领导者**，因为你日志不完整，别人都会拒绝给你投票
- raft 中日志必须是连续的
...
比如集群中原来是 A B C 三个节点的集群，现在加上了 D E，AB 和 C 形成了分区错误，那么此时 A 在 AB 中就是大多数，所以 A 是领导者，CDE 在 CDE 中获得了大多数，所以 C 也变成了领导者。

***我们使用单节点变更可以解决这个问题***

单节点变更，顾名思义，就是一次只变更一个节点来实现成员的变更。当然，如果你要变更多个节点，你需要执行多次的单次变更操作，从三个扩充到五个节点，你需要变更两次，从三个变成四个，从四个变成五个

举个例子，集群的配置是 [A,B,C]，我们向集群中添加了 D 节点，那么配置变成了 [A,B,C,D]

- 首先，领导者 A 向新节点 D 同步数据
- 领导者 A 将新配置 [A,B,C,D] 作为一个日志项，复制到新配置中所有节点 a b c d 上，然后通过新配置的日志项提交到本地状态机，完成单节点变更

如果你每次都只更新一个节点，那么，不管旧的集群配置是怎么组成的，旧配置的 “大多数” 和新配置的 “大多数” 都会有一个节点是重叠的。也就是说，不会同时存在旧配置和新配置 2 个 “大多数”，因为每一个节点都拥有这个配置单，节点仅存在一个配置单，拥有两个领导者是不可能的

如果遇到了分区错误，或者节点故障的情况下，并且执行并发单点变更，那么可能会出现一次单点变更尚未完成，新的单点又开始执行的情况，这个时候可能会存在两个节点，为了避免这种情况，我们可以引入一个空白日志，当领导者将空白日志提交后才能再次执行成员变更，有点像是加了一个锁的意思。
